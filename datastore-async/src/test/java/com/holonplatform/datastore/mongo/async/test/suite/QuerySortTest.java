...
public class QuerySortTest extends AbstractDatastoreOperationTest {
	...
	public void testSorts() {
		...
		long count = getDatastore().bulkInsert(TARGET, SET1)
				...
				.thenCompose(x -> getDatastore().query().target(TARGET).sort(STR1.desc()).sort(INT.desc()).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp3", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET).sort(STR1.desc()).sort(INT.asc()).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp3", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET).sort(INT.asc().and(STR1.desc())).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp1", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET).sort(STR1.asc()).list(STR1)).thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp1", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET).sort(STR1.desc()).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp3", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET).sort(QuerySort.asc(INT)).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp1", res.get(0));
				}).thenCompose(x -> getDatastore().query().target(TARGET)
						.sort(QuerySort.of(STR1, SortDirection.DESCENDING)).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp3", res.get(0));
				})
				.thenCompose(
						x -> getDatastore().query().target(TARGET).sort(QuerySort.of(STR1.desc(), INT.asc())).list(STR1))
				.thenAccept(res -> {
					assertEquals(3, res.size());
					assertEquals("bktfp3", res.get(0));
				})

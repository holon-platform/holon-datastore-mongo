...
public class QueryRestrictionTest extends AbstractDatastoreOperationTest {
	...
	public void testRestrictions() {
		...
		final Mono<Long> op = getDatastore().bulkInsert(TARGET, SET1)
				...
				.then(getDatastore().query(TARGET).sort(INT.asc()).limit(2).list(STR)).doOnSuccess(values -> {
					assertEquals(2, values.size());
					assertEquals("bktfp1", values.get(0));
					assertEquals("bktfp2", values.get(1));
				}).then(getDatastore().query(TARGET).sort(INT.asc()).limit(3).offset(2).list(STR))
				.doOnSuccess(values -> {
					assertEquals(3, values.size());
					assertEquals("bktfp3", values.get(0));
					assertEquals("bktfp4", values.get(1));
					assertEquals("bktfp5", values.get(2));
				}).then(getDatastore().query(TARGET).sort(INT.asc()).limit(2).offset(4).list(STR))
				.doOnSuccess(values -> {
					assertEquals(1, values.size());
					assertEquals("bktfp5", values.get(0));
				})
				.then(getDatastore().bulkDelete(TARGET).filter(STR.in("bktfp1", "bktfp2", "bktfp3", "bktfp4", "bktfp5"))

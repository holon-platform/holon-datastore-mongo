...
public class AsyncMongoRefresh extends AbstractAsyncRefresh {
	...
	public CompletionStage<PropertyBox> execute() {
		return CompletableFuture.supplyAsync(() -> {
			...
			// build context
			return DocumentOperationContext.create(context, collection, null);
		}).thenCompose(context -> {
			// id property
			...
			if (context.getDocument() == null) {
				throw new DataAccessException("No document found using id property ["
						+ context.getDocumentContext().getDocumentIdProperty().orElse(null) + "]");
			}
			// trace
			context.getOperationContext().trace("Refreshed document", DocumentSerializer.getDefault()
					.toJson(context.getCollection().getCodecRegistry(), context.getDocument()));
			// build operation result
			return context.getDocumentContext()
					.resolveOrFail(DocumentValue.create(context.getDocument()), PropertyBoxValue.class).getValue();
		});
	}

...
public class UpdateTest extends AbstractDatastoreOperationTest {
	...
	public void testUpdate() {
		...
		final Mono<Long> op = getDatastore()
				...
				.then(getDatastore().update(TARGET,
	...
	public void testUpdateMissingId() {
		getDatastore().update(TARGET, PropertyBox.builder(SET1).set(STR1, TestValues.STR1).build()).block();
	}
	...
	public void testUpdateNested() {
		...
		final Mono<Long> op = getDatastore()
				...
				.then(getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET6)).map(value -> {
					...
					return value;
				}).flatMap(value -> getDatastore().update(TARGET, value))
				.doOnSuccess(r -> assertEquals(1, r.getAffectedCount()))
	...
	public void testUpdateNestedPropertyBox() {
		...
		final Mono<Long> op = getDatastore()
				...
				.doOnSuccess(r -> assertEquals(1, r.getAffectedCount()))
				.then(getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET7)).doOnSuccess(value -> {
					assertNotNull(value);
					assertEquals(oid, value.getValue(ID));
					assertEquals("upd", value.getValue(STR1));
					assertEquals(EnumValue.FIRST, value.getValue(ENM));

...
The link:{coreapidir}/com/holonplatform/reactor/datastore/transaction/ReactiveTransaction.html[ReactiveTransaction^] interface is used as transaction representation and provides methods to inspect the transaction status and perform the *commit* and *rollback* operations.

...
public class BulkUpdateTest extends AbstractDatastoreOperationTest {
	...
	public void testBulkUpdate() {
		...
		final Mono<Long> op = getDatastore().insert(TARGET, value1)
				...
				.then(getDatastore().bulkUpdate(TARGET).filter(ID.in(oid1, oid2, oid3)).set(DBL, 77.99d).setNull(INT)
						.execute())
				.doOnSuccess(r -> assertEquals(3, r.getAffectedCount()))
				.then(getDatastore().query(TARGET).filter(ID.eq(oid1)).findOne(INT)).doOnSuccess(r -> {
					assertNull(r);
				}).then(getDatastore().query(TARGET).filter(ID.eq(oid2)).findOne(INT)).doOnSuccess(r -> {
					assertNull(r);
				}).then(getDatastore().query(TARGET).filter(ID.eq(oid3)).findOne(INT)).doOnSuccess(r -> {
					assertNull(r);
				}).then(getDatastore().query(TARGET).filter(ID.eq(oid1)).findOne(DBL)).doOnSuccess(dv -> {
					assertEquals(Double.valueOf(77.99d), dv);
				}).then(getDatastore().query(TARGET).filter(ID.eq(oid2)).findOne(DBL)).doOnSuccess(dv -> {
					assertEquals(Double.valueOf(77.99d), dv);
				}).then(getDatastore().query(TARGET).filter(ID.eq(oid3)).findOne(DBL)).doOnSuccess(dv -> {
					assertEquals(Double.valueOf(77.99d), dv);
				}).then(getDatastore().bulkDelete(TARGET).filter(ID.in(oid1, oid2, oid3)).execute()
	...
	public void testBulkUpdateDate() {
		...
		final Mono<Long> op = getDatastore()
				...
				.doOnSuccess(r -> assertEquals(1, r.getAffectedCount()))
				.then(getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET1)).doOnSuccess(uvalue -> {

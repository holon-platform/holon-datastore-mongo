...
/**
 * Registrar for MongoDB sync Datastore bean registration using the {@link EnableMongoDatastore} annotation.
 * 
 * @since 5.2.0
 */
public class MongoSyncDatastoreRegistrar extends AbstractConfigPropertyRegistrar implements BeanClassLoaderAware {
	...
	public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry registry) {
		...
		if (!annotationMetadata.isAnnotated(EnableMongoDatastore.class.getName())) {
			// ignore call from sub classes
			return;
		}

		Map<String, Object> attributes = annotationMetadata
				.getAnnotationAttributes(EnableMongoDatastore.class.getName());
		...
		// register Datastore
		registerSyncDatastore(registry, getEnvironment(), dataContextId, mongoClientBeanName, defaultConfig,
				beanClassLoader);
	}

	/**
	 * Register a {@link MongoDatastore} bean.
	 ...
	 */
	public static String registerSyncDatastore(BeanDefinitionRegistry registry, Environment environment,
		...
		final Class<?> datastoreClass = DefaultMongoDatastore.class;
		...
		String beanName = BeanRegistryUtils.buildBeanName(dataContextId,
				EnableMongoDatastore.DEFAULT_DATASTORE_BEAN_NAME);
		...
		log.append("Registered MongoDB Sync Datastore bean with name \"");
		log.append(beanName);
		log.append("\"");
		if (dataContextId != null) {

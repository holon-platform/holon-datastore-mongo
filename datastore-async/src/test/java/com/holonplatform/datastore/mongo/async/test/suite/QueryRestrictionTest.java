...
public class QueryRestrictionTest extends AbstractDatastoreOperationTest {
	...
	public void testRestrictions() {
		...
		OperationResult result = getDatastore().bulkInsert(TARGET, SET1)
				...
				.add(PropertyBox.builder(SET1).set(STR, "bktfp5").set(INT, 5).build()).execute().toCompletableFuture().join();
		assertEquals(5, result.getAffectedCount());

		List<String> values = getDatastore().query(TARGET).sort(INT.asc()).limit(2).list(STR).toCompletableFuture().join();
		assertEquals(2, values.size());
		assertEquals("bktfp1", values.get(0));
		assertEquals("bktfp2", values.get(1));

		values = getDatastore().query(TARGET).sort(INT.asc()).limit(3).offset(2).list(STR).toCompletableFuture().join();
		assertEquals(3, values.size());
		assertEquals("bktfp3", values.get(0));
		assertEquals("bktfp4", values.get(1));
		assertEquals("bktfp5", values.get(2));

		values = getDatastore().query(TARGET).sort(INT.asc()).limit(2).offset(4).list(STR).toCompletableFuture().join();
		assertEquals(1, values.size());
		assertEquals("bktfp5", values.get(0));

		result = getDatastore().bulkDelete(TARGET).filter(STR.in("bktfp1", "bktfp2", "bktfp3", "bktfp4", "bktfp5"))

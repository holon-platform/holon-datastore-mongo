...
import com.holonplatform.datastore.mongo.spring.EnableMongoReactiveDatastore;
...
/**
 * Registrar for MongoDB reactive Datastore bean registration using the {@link EnableMongoReactiveDatastore} annotation.
 * 
 * @since 5.2.0
 */
public class MongoReactiveDatastoreRegistrar extends AbstractConfigPropertyRegistrar implements BeanClassLoaderAware {
	...
	public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry registry) {
		...
		if (!annotationMetadata.isAnnotated(EnableMongoReactiveDatastore.class.getName())) {
			// ignore call from sub classes
			return;
		}

		Map<String, Object> attributes = annotationMetadata
				.getAnnotationAttributes(EnableMongoReactiveDatastore.class.getName());
		...
		// register Datastore
		registerReactiveDatastore(registry, getEnvironment(), dataContextId, mongoClientBeanName, defaultConfig,
				beanClassLoader);
	}

	/**
	 * Register a {@link ReactiveMongoDatastore} bean.
	 ...
	 */
	public static String registerReactiveDatastore(BeanDefinitionRegistry registry, Environment environment,
		...
		final Class<?> datastoreClass = DefaultReactiveMongoDatastore.class;
		...
		String beanName = BeanRegistryUtils.buildBeanName(dataContextId,
				EnableMongoReactiveDatastore.DEFAULT_DATASTORE_BEAN_NAME);
		...
		log.append("Registered MongoDB Reactive Datastore bean with name \"");
		log.append(beanName);
		log.append("\"");
		if (dataContextId != null) {

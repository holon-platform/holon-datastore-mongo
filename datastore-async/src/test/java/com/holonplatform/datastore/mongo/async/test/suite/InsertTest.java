...
public class InsertTest extends AbstractDatastoreOperationTest {
	...
	public void testInsertBringBackIdType() {
		...
		result = getDatastore().insert(TARGET, value, DefaultWriteOption.BRING_BACK_GENERATED_IDS).toCompletableFuture().join();
	...
	public void testInsertNested() {
		...
		value = getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET6).toCompletableFuture().join().orElse(null);
		assertNotNull(value);
	...
	public void testInsertNestedPropertyBox() {
		...
		value = getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET7).toCompletableFuture().join().orElse(null);
		assertNotNull(value);
	...
	public void testInsertNestedMixed() {
		...
		value = getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET8).toCompletableFuture().join().orElse(null);
		assertNotNull(value);
	...
	public void testInsertPropertyBoxCollections() {
		...
		value = getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET10).toCompletableFuture().join().orElse(null);
		assertNotNull(value);

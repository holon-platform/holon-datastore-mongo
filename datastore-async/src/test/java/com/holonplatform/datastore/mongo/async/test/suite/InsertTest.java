...
import static com.holonplatform.datastore.mongo.async.test.data.ModelTest.C_PBX;
...
import static com.holonplatform.datastore.mongo.async.test.data.ModelTest.SET10;
...
public class InsertTest extends AbstractDatastoreOperationTest {
	...
	public void testInsert() {
		...
		long count = getDatastore().insert(TARGET, PropertyBox.builder(SET1).set(ID, oid).set(STR, TestValues.STR)
				...
				.set(C_ENM, TestValues.C_ENM).set(C_LNG, TestValues.C_LNG).set(NBL, true).build()).thenAccept(r -> {
				...
				}).thenCompose(value -> getDatastore().delete(TARGET, value)).thenAccept(r -> {
					assertEquals(1, r.getAffectedCount());
				}).thenCompose(v -> getDatastore().query(TARGET).filter(ID.eq(oid)).count()).toCompletableFuture()
				.join();
	...
	public void testInsertIdType() {
		...
		long count = getDatastore()
				...
				.thenApply(r -> {
					assertEquals(1, r.getAffectedCount());
					return oid;
				}).thenCompose(id -> getDatastore().bulkDelete(TARGET).filter(ID4.eq(id.toString())).execute())
	...
	public void testInsertBringBackIdType() {
		...
		long count = getDatastore().insert(TARGET, PropertyBox.builder(SET4).set(STR, TestValues.STR).build())
				.thenApply(r -> {
					...
					return oid;
				}).thenCompose(oid -> getDatastore().query(TARGET).filter(ID4.eq(oid)).findOne(ID4))
				.thenApply(result -> {
					assertTrue(result.isPresent());
					return result.get();
				}).thenCompose(oid -> getDatastore().bulkDelete(TARGET).filter(ID4.eq(oid)).execute())
				.thenApply(r -> r.getAffectedCount()).toCompletableFuture().join();
	...
	public void testBringBackIds() {
		...
		long count = getDatastore().insert(TARGET, value, DefaultWriteOption.BRING_BACK_GENERATED_IDS).thenApply(r -> {
			...
			return oid;
		}).thenCompose(oid -> getDatastore().bulkDelete(TARGET).filter(ID.eq(oid)).execute())
				.thenApply(r -> r.getAffectedCount()).toCompletableFuture().join();
	...
	public void testInsertNested() {
		...
		long count = getDatastore()
				...
				.thenAccept(r -> assertEquals(1, r.getAffectedCount())).thenCompose(v -> {
					return getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET6);
				}).thenApply(res -> {
		...
		long count = getDatastore()
				.insert(TARGET,
						PropertyBox.builder(SET7).set(ID, oid).set(STR, "testn").set(ENM, EnumValue.FIRST)
								...
								.build())
				.thenAccept(r -> assertEquals(1, r.getAffectedCount())).thenCompose(v -> {
					return getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET7);
				}).thenApply(res -> {
					assertTrue(res.isPresent());
					return res.get();
				}).thenApply(value -> {
	...
	public void testInsertNestedMixed() {
		...
		long count = getDatastore()
				...
				.thenAccept(r -> assertEquals(1, r.getAffectedCount())).thenCompose(v -> {
					return getDatastore().query(TARGET).filter(ID.eq(oid)).findOne(SET8);
				}).thenApply(res -> {
					assertTrue(res.isPresent());
					return res.get();
				}).thenApply(value -> {
